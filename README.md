...
본 프로젝트는 무거운 표준 YOLOv5s 모델(702만 파라미터)을 **Jetson Nano 4GB RAM 및 640x480 웹캠 해상도** 환경에 완벽하게 맞추어 **1.76M 파라미터(75% 감축, 3.9MB)**로 다이어트 설계한 커스텀 객체 탐지 모델입니다.

공식 사전 학습 뇌(`yolov5n.pt`)의 무게중심을 100% 온전히 승계받아 **주먹 97.6% / 보자기 99.1% mAP50**의 최고 수준 정확도를 달성하며, **640x480 직사각형 학습(`--img 480 640 --rect`) 및 TensorRT FP16 가속을 통해 45~55 FPS의 고속 실시간 추론**을 구현합니다.
...
* **📹 640x480 웹캠 1대1 직사각형 해상도 최적화 (`--img 480 640 --rect`):**
...
├── weights_170_640x480/
│   ├── best.pt                         # 🔥 640x480 직사각형 최적 가중치 (3.9MB, 1.76M Params)
...
| 지표 | 기존 YOLOv5s | 커스텀 슬림 모델 (`weights_170_640x480`) | 개선 및 감축 효과 |
| :--- | :---: | :---: | :---: |
| **파라미터 수 (Parameters)** | 7,025,000 개 (7.02M) | **1,763,224 개 (1.76M)** | **약 75% 감축** 📉 |
| **가중치 용량 (Weight Size)** | ~15.0 MB | **3.9 MB** | **약 74% 다이어트** 📦 |
| **입력 해상도 (Resolution)** | 640x640 (정사각형) | **480x640 (직사각형 Rect)** | **웹캠 4:3 비율 1대1 매칭** 📹 |
...
python3 export.py --weights weights_170_640x480/best.pt --img 480 640 --batch 1 --include onnx

# 2. trtexec 가속 컴파일
/usr/src/tensorrt/bin/trtexec --onnx=weights_170_640x480/best.onnx --saveEngine=best.engine --fp16 --workspace=1024 --avgRuns=1
```

### 3) 실시간 640x480 웹캠 추론 테스트
```bash
python3 detect.py --weights best.engine --img 480 640 --conf 0.25 --source 0 --data dataset.yaml --classes 1 2
```
